...
class EducationPolicy:
    ...
    def lesson_allowed(self, lesson: Lesson) -> bool:
        confidence = lesson.metadata.get("confidence", 1.0)
        try: value = float(confidence)
        except (TypeError, ValueError): return False
        if not 0.0 <= value <= 1.0: return False
        return value >= self.min_lesson_confidence and all(u.strip() for u in lesson.evidence_urls) if lesson.evidence_urls else value >= self.min_lesson_confidence

    def course_allowed(self, course: Course) -> bool:
        return (not course.premium or self.allow_premium_offers) and course.price_minor >= 0 and len(course.currency) == 3

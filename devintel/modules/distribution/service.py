...
from .natural import NaturalPublishingCandidate, NaturalPublishingEngine
...
class DistributionService:
    ...
    def publish(self, request: PublishRequest, *, owner_approved: bool = False) -> DistributionResult:
        ...
        candidate = NaturalPublishingCandidate(
            request.value_score, request.confidence, request.freshness, request.urgency
        )
        ...
        policy = self.policy.evaluate(destination, value_score=request.value_score, confidence=request.confidence,
        ...
        self.audit.record(AuditRecord("distribution.completed", action=action.action, success=result.accepted,
                                      details={"destination": destination.destination_id, "reason": result.reason, "message_id": result.message_id}))

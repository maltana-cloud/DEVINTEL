...
class GrowthSpecialist:
    ...
    def __init__(self, plugins: PluginService | None = None, engine: GrowthEngine | None = None) -> None:
        self.plugins = plugins or PluginService(); self.engine = engine or GrowthEngine()
        self.plugins.register(PluginManifest(self.plugin_id, "Growth Specialist", "1.1.1", "Discovers scoped audience needs and creates useful awareness plans.", ("growth.discover", "growth.plan"), ("growth.read",), PluginRisk.LOW))
        self.attach()
    def attach(self) -> None:
        def run(action: PluginAction) -> GrowthSpecialistResult:
            ...
            run = self.engine.run(action.scope_id, scoped)
            return GrowthSpecialistResult(action.scope_id, run.plans)
    ...
    def execute(self, scope_id: str, query: str, source: GrowthSource) -> GrowthSpecialistResult:
        ...
        result = self.plugins.execute(PluginAction(self.plugin_id, "growth.scan", scope_id.strip(), PluginRisk.LOW, "bounded growth discovery", payload={"query": query.strip(), "source": source}))
